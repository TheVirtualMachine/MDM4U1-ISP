...
makeBoxPlot <- function(categoryData, categoryName, categoryTitle) {
	...
	dataPlot <- dataPlot + labs(x = categoryTitle, y = "Purity") # Give axes and legend proper labels.
	...
	dataPlot <- dataPlot + geom_boxplot() # Add the data points.
	dataPlot <- dataPlot + labs(x = categoryTitle, y = "Orthodoxy") # Give axes and legend proper labels.
	dataPlot <- dataPlot + theme(legend.position = "none")
