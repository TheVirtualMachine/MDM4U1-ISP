...
data <- data[data$stream %in% names(table(data$stream))[table(data$stream) > 1],] # Remove all streams only occurring once, as these data are not helpful for a box plot.

# Create the purity plot.
dataPlot <- ggplot(data, aes(stream, purity, fill=stream), ) # Setup the plot.
...
dataPlot <- ggplot(data, aes(stream, orthodoxy, fill=stream), ) # Setup the plot.
dataPlot <- dataPlot + geom_boxplot() # Add the data points.
